  ...
  <div class="preference">
    ...
    <scroll-view scroll-x="true" class="flexBox">
      <ul>
        <li
          ...
          :key="index"
          :class="{'on':item.cityId == select}"
          @click="change(item.cityId)"
        >{{item.city}}</li>
      </ul>
    </scroll-view>
    <div v-for="(city,cityid) in preference" :key="cityid" class="showHouse">
      <div v-if="city.cityId == select" class="cho">
        <div
          class="house"
          v-for="(house,houseid) in listHouse"
          :key="houseid"
          v-show="houseid<defalut"
          @click="router(house.houseId,city.city)"
        >
          <div class="pic">
            <img :src="house.showPic" alt />
          </div>
          <div class="info">
        ...
        <div class="more" @click="showMore(houseList.length)" v-show="ifMore">
          <button>显示更多{{city.city}}的房源</button>
        </div>
      </div>
    </div>
  </div>
...
<script>
export default {
  props: ["preference", "houseList", "listHouse"],
  data() {
    return {
      select: 1,
      ifMore: true,
  ...
  methods: {
    ...
    showMore(len) {
      (this.defalut = len), (this.ifMore = false);
    },
    router(id, city) {
      if (id == 1 && city == "苏州") {
  ...
  watch: {
    select(val) {
        this.listHouse = this.houseList.filter(item => item.cityId == this.select);
